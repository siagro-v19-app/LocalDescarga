	...
	return Controller.extend("br.com.idxtecLocalDescarga.controller.LocalDescarga", {
		onInit: function(){
			...
			this.getModel().attachMetadataLoaded(function(){
				...
				var oTable = oView.byId("tableLocal");
				var oColumn = oView.byId("columnDescricao");
				
				oTable.sort(oColumn);
				oView.byId("tableLocal").getBinding("rows").filter(oFilter, "Application");
			});
		},
		...
		filtraLocal: function(oEvent){
			...
			this.getView().byId("tableLocal").getBinding("rows").filter(aFilters, "Application");
		},
